...
export type NetworkIdentityEvent = {id:string;kind:"identity_seen"|"address_changed";name:string;source_name:string;old_value:string|null;new_value:string|null;occurred_at:string;acknowledged_at:string|null;severity:"low"|"medium"};
...
export const api = {
  ...
  historicalReport: (days:number) => request<HistoricalReport>(`/api/v1/reports/history?days=${days}`),
  applications: () => request<ApplicationIntegration[]>("/api/v1/applications"),
  applicationEvents: () => request<ApplicationEvent[]>("/api/v1/applications/events"),
  acknowledgeApplicationEvent: (id:string,csrfToken:string) => request<ApplicationEvent>(`/api/v1/applications/events/${id}/acknowledge`,{method:"POST",headers:{"X-CSRF-Token":csrfToken}}),
  createApplication: (payload:Record<string,unknown>,csrfToken:string) => request<ApplicationIntegration>("/api/v1/applications",{method:"POST",headers:{"X-CSRF-Token":csrfToken},body:JSON.stringify(payload)}),
  ...
  acknowledgeNetworkActivity: (id:string,csrfToken:string) => request<NetworkIdentityEvent>(`/api/v1/sources/network-activity/${id}/acknowledge`,{method:"POST",headers:{"X-CSRF-Token":csrfToken}}),
