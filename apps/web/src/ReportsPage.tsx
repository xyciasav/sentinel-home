...
export function ReportsPage({report,refresh}:{report:OverviewReport|null;refresh:()=>Promise<void>}) {
  ...
  return <>
    ...
    <section className="status-grid report-cards">
      ...
      <article className="status-card"><p>Agent coverage</p><strong className={report.agents_connected===report.agents_total?"green":""}>{report.agents_connected} / {report.agents_total}</strong><small>{report.agents_current} running agent v0.6.0 + executor v0.3.1</small></article>
